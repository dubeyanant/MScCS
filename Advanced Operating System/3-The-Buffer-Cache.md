...
- [Scenarios for Retrieval of a Buffer](#scenarios-for-retrieval-of-a-buffer)
- [Reading and Writing Disk Blocks](#reading-and-writing-disk-blocks)

## Buffer Headers

Kernel reads and writes directly to and from the disk, but system response time and throughput will be poor because of slow transfer rate of the disk. The kernel therefore minimizes this time by keeping a pool of data buffers, called **the buffer cache**, which contains recently used data in the cache memory.
...
<img src="Diagrams/Screen_Shot_2017-06-07_at_10.27.45_PM.png" alt="Buffer header" style="zoom:80%;" />
...
When the kernel accesses a disk block, it searches for the buffer with the appropriate device-block number combination. Rather than searching the entire buffer pool, it organizes the buffers into separate queues, *hashed* as a function of the device and block number. The hash queues are also doubly linked circular lists. A hashing function which uniformly distributes the buffers across the lists is used. But it also has to be simple so that the performance does not suffer.
